...
local CornerX, CornerZ = 161.25, -163.5
local EndBoundX = 91.25
local GridSpacing = 3.5

PlaceTab:CreateInput({ Name = "Start Corner X", PlaceholderText = "161.25", Callback = function(T) CornerX = tonumber(T) or 161.25 end })
PlaceTab:CreateInput({ Name = "Start Corner Z", PlaceholderText = "-163.5", Callback = function(T) CornerZ = tonumber(T) or -163.5 end })
PlaceTab:CreateInput({ Name = "End Boundary X (Limit Edge)", PlaceholderText = "91.25", Callback = function(T) EndBoundX = tonumber(T) or 91.25 end })
    ...
    Callback = function(Val)
        if Val then
            ...
            ViewRender = RunService.RenderStepped:Connect(function()
                local widthX = math.abs(CornerX - EndBoundX)
                ViewPart.Size = Vector3.new(widthX, 1, 60)
                local midX = (CornerX + EndBoundX) / 2
                ViewPart.Position = Vector3.new(midX, LocalPlayer.Character and LocalPlayer.Character.PrimaryPart and LocalPlayer.Character.PrimaryPart.Position.Y - 3 or 0, CornerZ + 30)
            end)
        else
        ...
        if _G.AutoLoop then
            task.spawn(function()
                ...
                while _G.AutoLoop do
                    ...
                    if currentId then
                        -- Dynamically maps out exactly how many missiles cleanly fit inside the X boundary line
                        local allowedDistance = math.abs(CornerX - EndBoundX)
                        local MaxPerRow = math.max(1, math.floor(allowedDistance / GridSpacing))
                        
                        local row = math.floor(count / MaxPerRow)
                        local col = count % MaxPerRow
                        
                        local targetX = CornerX - (col * GridSpacing)
                        local targetZ = CornerZ + (row * GridSpacing) 
