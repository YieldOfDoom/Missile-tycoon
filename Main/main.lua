...
-- [RESTORED: VOZEX NETWORK VERIFICATION]
...
if httpRequest then
    ...
    if not success or res.StatusCode ~= 200 then return end
else
    return
end
...
for _, b in pairs(Buildings) do
    BuyTab:CreateToggle({
        ...
        Callback = function(Value)
            ...
            if _G[b.Def] then
                task.spawn(function()
                    while _G[b.Def] do runBuy(b.ID, b.Shop, b.Def, b.Req) task.wait(2) end
                end)
            end
        end
    })
...
for _, m in pairs(MilitaryItems) do
    MilitaryTab:CreateToggle({
        ...
        Callback = function(Value)
            ...
            if _G[m.Def] then
                task.spawn(function()
                    while _G[m.Def] do runBuy(m.ID, m.Shop, m.Def, m.Req) task.wait(3) end
                end)
            end
        end
    })
...
local RowSpacing, ColSpacing = 8, 8

PlaceTab:CreateInput({ Name = "Corner 1: X Boundary", PlaceholderText = "0", Callback = function(Text) X1 = tonumber(Text) or 0 end })
PlaceTab:CreateInput({ Name = "Corner 1: Z Boundary", PlaceholderText = "0", Callback = function(Text) Z1 = tonumber(Text) or 0 end })
PlaceTab:CreateInput({ Name = "Corner 2: X Boundary", PlaceholderText = "0", Callback = function(Text) X2 = tonumber(Text) or 0 end })
PlaceTab:CreateInput({ Name = "Corner 2: Z Boundary", PlaceholderText = "0", Callback = function(Text) Z2 = tonumber(Text) or 0 end })
...
PlaceTab:CreateButton({
    ...
    Callback = function()
        ...
        if math.abs(maxX - minX) < 2 or math.abs(maxZ - minZ) < 2 then
            return Rayfield:Notify({Title="Invalid Bounds", Content="Make area sizes wider!", Duration=3})
        end
        
        Rayfield:Notify({Title="Grid Engaged", Content="Deploying inventory items into matrix...", Duration=3})
        
        task.spawn(function()
            for currentX = minX, maxX, RowSpacing do
                for currentZ = minZ, maxZ, ColSpacing do
                    local targetTool = nil
                    local bp = LocalPlayer:FindFirstChild("Backpack")
                    if bp then
                        for _, item in ipairs(bp:GetChildren()) do
                            if item:IsA("Tool") and (item.Name:lower():match("rocket") or item:GetAttribute("ObjectId")) then
                                targetTool = item break
                            end
                        end
                    end
                    ...
                    if targetTool then
                        local objId = targetTool:GetAttribute("ObjectId") or targetTool.Name:match("^%d+_%d+_%d+") or targetTool.Name
                        if targetTool.Parent == bp and character:FindFirstChildOfClass("Humanoid") then
                        ...
                        local slots, storageId = getInventorySpacesAndStorage()
                        if slots > 0 then 
                            runTakeRocket("Rocket1", 1, storageId)
                            task.wait(0.3)
                        else
                            task.wait(0.5)
                        end
            ...
            Rayfield:Notify({Title="Matrix Complete", Content="All available layout lines deployed.", Duration=3})
        end)
    end
})
...
task.spawn(function()
    while task.wait(5) do
        ...
        if _G.RichBypassEnabled then
            for _, p in pairs(Players:GetPlayers()) do if p ~= LocalPlayer and GetCash(p) >= _G.RichBypassLimit then triggerHop = true break end end
        end
        if _G.FindNoobsEnabled and not triggerHop then
            ...
            if not foundNoob and #Players:GetPlayers() > 1 then triggerHop = true end
        end
        if triggerHop then ServerHop() task.wait(10) end
    end
end)

-- [RESTORED: FINAL COUNTRY HANDLER]
task.spawn(function() pcall(function() ReplicatedStorage:WaitForChild("CityBuilderRockets"):WaitForChild("Remotes"):WaitForChild("ConfirmCountrySelectionEvent"):FireServer("NONE") end) end)
