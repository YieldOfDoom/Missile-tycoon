...
PlaceTab:CreateSection("Selection Matrix Geometry")

-- Track boundary configurations
local Corner1X, Corner1Z = 161.25, -163.5
local Corner2X, Corner2Z = 91.25, -93.5
local GridSpacing = 3.5

PlaceTab:CreateInput({ Name = "Corner 1 X", PlaceholderText = "161.25", Callback = function(T) Corner1X = tonumber(T) or 161.25 end })
PlaceTab:CreateInput({ Name = "Corner 1 Z", PlaceholderText = "-163.5", Callback = function(T) Corner1Z = tonumber(T) or -163.5 end })
PlaceTab:CreateInput({ Name = "Corner 2 X", PlaceholderText = "91.25", Callback = function(T) Corner2X = tonumber(T) or 91.25 end })
PlaceTab:CreateInput({ Name = "Corner 2 Z", PlaceholderText = "-93.5", Callback = function(T) Corner2Z = tonumber(T) or -93.5 end })
...
PlaceTab:CreateToggle({
    Name = "👀 Show Selection Box Preview",
    CurrentValue = false,
    Callback = function(Val)
        if Val then
            ...
            -- Original physical setup to project the neon outline framework clearly
            ...
            ViewBox.Color3 = Color3.fromRGB(255, 0, 0)
            ViewBox.LineThickness = 0.15
            ViewBox.Adornee = ViewPart
            ViewBox.Parent = ViewPart

            ViewRender = RunService.RenderStepped:Connect(function()
                local minX = math.min(Corner1X, Corner2X)
                local maxX = math.max(Corner1X, Corner2X)
                local minZ = math.min(Corner1Z, Corner2Z)
                local maxZ = math.max(Corner1Z, Corner2Z)
                
                local sizeX = maxX - minX
                local sizeZ = maxZ - minZ
                
                ViewPart.Size = Vector3.new(sizeX, 4, sizeZ)
                ViewPart.Position = Vector3.new((minX + maxX) / 2, LocalPlayer.Character and LocalPlayer.Character.PrimaryPart and LocalPlayer.Character.PrimaryPart.Position.Y - 1 or 0, (minZ + maxZ) / 2)
            end)
        else
...
PlaceTab:CreateToggle({
    Name = "🚀 Start Selection Auto-Grid",
    CurrentValue = false,
    Callback = function(Value)
        _G.AutoLoop = Value
        ...
        if _G.AutoLoop then
            task.spawn(function()
                ...
                while _G.AutoLoop do
                    ...
                    local currentId = nil
                    -- Your high-speed memory scanning mechanism
                    ...
                    if currentId then
                        ...
                        local MaxPerRow = math.max(1, math.floor(widthX / GridSpacing))
                        
                        local row = math.floor(count / MaxPerRow)
                        local col = count % MaxPerRow
                        
                        -- Calculates placement paths relative to the box layout
                        local targetX = maxX - (col * GridSpacing)
                        local targetZ = minZ + (row * GridSpacing) 
                        ...
                        if targetZ <= maxZ then
                            ...
                            Rayfield:Notify({Title="Grid Full", Content="Selection boundary completely filled!", Duration=3})
                            _G.AutoLoop = false
                            break
                        end
                    end
...
print("Full Automation System Loaded!")
